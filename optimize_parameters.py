#!/usr/bin/env python3
"""
Re-run blob detection with better tuned parameters to match PDF samples.
"""
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
def detect_and_save(image_path, sigma0, k, n_scales, threshold, suffix=""):
    print(f"\n{'='*70}")
    print(f"Processing: {image_path}")
    print(f"Parameters: sigma0={sigma0}, k={k}, n_scales={n_scales}, threshold={threshold}")
    print(f"{'='*70}")
    ...
    print(f"  Scale range: [{sigma_list[0]:.2f}, {sigma_list[-1]:.2f}]")
    ...
    overlay_path = OUTPUT_DIR / f'{base_name}_blobs{suffix}.png'
    ...
    json_path = OUTPUT_DIR / f'{base_name}_blobs{suffix}.json'
...
# Test different thresholds to find better parameters
print("="*70)
print("OPTIMIZING PARAMETERS - TESTING DIFFERENT THRESHOLDS")
print("="*70)

# For butterfly - the issue is we're detecting too many small features
# Let's try higher thresholds and narrower scale range
img_path = 'butterfly.jpg'
if Path(img_path).exists():
    print(f"\n\nTESTING BUTTERFLY WITH DIFFERENT PARAMETERS:")
    print("="*70)

    # Original was detecting 1995 blobs - way too many!
    # PDF sample shows maybe 50-150 blobs (the white spots)

    # Try much higher threshold
    test_configs = [
        (4.0, 1.3, 10, 0.01, "_thresh0.01"),   # 10x higher threshold
        (4.0, 1.3, 10, 0.015, "_thresh0.015"), # 15x higher
        (4.0, 1.3, 10, 0.02, "_thresh0.02"),   # 20x higher
        (5.0, 1.4, 8, 0.015, "_narrow_scales"), # Narrower scale range
    ]

    for sigma0, k, n_scales, threshold, suffix in test_configs:
        n_blobs = detect_and_save(img_path, sigma0, k, n_scales, threshold, suffix)

print(f"\n{'='*70}")
print(f"PARAMETER OPTIMIZATION COMPLETE!")
print(f"{'='*70}")
print("\nCheck outputs/ folder for results with different parameters.")
print("Look for the version that best matches the PDF sample (~50-150 blobs).")
